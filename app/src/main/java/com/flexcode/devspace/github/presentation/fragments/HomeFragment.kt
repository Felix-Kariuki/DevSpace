...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        getUserDetailsViewModel.searchQuery.value?.let { getUserDetails(it) }
        val username = "Felix-Kariuki"
...
//                        binding.animationView.isGone = true
    ...
    private fun getUserDetails(s: String) {
        viewLifecycleOwner.lifecycleScope.launchWhenCreated {
            getUserDetailsViewModel.getUserDetails(
                "Felix-Kariuki"
            ).collectLatest{response ->
                when(response){
                    is Resource.Success -> {
                        binding.apply {
//                            ivUserProfile.load("${response.data?.avatar_url}"){
//                                crossfade(true)
